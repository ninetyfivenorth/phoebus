    ...
    if ($_addonManifest == false) {
        ...
        if ($_mode == 1) {
            ...
            if ($_SERVER["HTTP_X_FORWARDED_HOST"] == 'dev.addons.palemoon.org') {
                $_addonManifest["baseurl"] = 'https://dev.addons.palemoon.org/datastore/' . $_addonType . 's/' . $_addonSlug . '/' . $_addonManifest['description'];
            }
            else {
                $_addonManifest["baseurl"] = 'https://addons.palemoon.org/phoebus/datastore/' . $_addonType . 's/' . $_addonSlug . '/' . $_addonManifest['description'];
            }
        }
        elseif ($_mode == 2) {
            unset($_addonManifest['id']);
